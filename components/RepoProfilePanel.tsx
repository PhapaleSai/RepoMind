...
export default function RepoProfilePanel({
  ...
  // Onboarding checkboxes are a per-repo progress tracker, so they're keyed by repositoryId
  // and persisted only in this browser — no server round trip needed for something this small.
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem(`repomind_onboarding_${repositoryId}`);
  ...
  function toggleStep(i: number) {
    setCheckedSteps((prev) => {
      ...
      try {
        window.localStorage.setItem(`repomind_onboarding_${repositoryId}`, JSON.stringify([...next]));
      } catch {
        // ignore
      }
      return next;
  ...
  return (
    <section
      className={`glass-panel spotlight-card rounded-2xl transition ${
        justAutoGenerated ? "ring-2 ring-fuchsia-400/40" : ""
      }`}
    ...
    >
      ...
      {open && (
        <div className="space-y-5 border-t border-white/10 px-5 py-5">
          ...
          <VulnerabilityScan repositoryId={repositoryId} />
          ...
          {profile && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="mb-2 flex items-center justify-between gap-1.5 text-xs font-medium uppercase tracking-wide text-white/40">
                  <span className="flex items-center gap-1.5">
                    <ListChecks className="h-3.5 w-3.5" /> Onboarding checklist
                  </span>
                  <span className="normal-case text-white/30">
                    {checkedSteps.size}/{profile.onboarding.length} done
                  </span>
                </h3>
                    ...
                    return (
                      <li key={i}>
                        <label className="flex cursor-pointer items-start gap-2 text-sm">
                          ...
                          />
                          <span className={done ? "text-white/30 line-through" : "text-white/80"}>{step}</span>
                        </label>
                      </li>
                    );
                  })}
