...
import { buildRepoGraph } from "@/lib/graph";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const graph = await buildRepoGraph(repositoryId);
    return NextResponse.json({ graph });
  } catch (err: any) {
    const status = err instanceof AccessDeniedError ? 403 : 500;
    return NextResponse.json({ error: err.message ?? "Graph generation failed" }, { status });
  }
}
